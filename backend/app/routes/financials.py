...
def get_monthly_summary(
    ...
    """
    Aggregate a full month's financials from live tables:
      - Revenue streams  → DailySummary
      - Expenses         → ExpenseEntry
      - Payroll          → WeeklyPayroll (weeks whose start falls in the month)
    Then runs compute_snapshot() for the full business-logic breakdown.
    """
    from calendar import monthrange
    start = date(year, month, 1)
    end   = date(year, month, monthrange(year, month)[1])

    # ── Revenue (DailySummary rows for each day in the month) ──
    summaries = (
    ...
    def ds(field: str) -> Decimal:
        return sum(getattr(s, field) or Decimal("0") for s in summaries)

    total_wash_set  = ds("total_wash_set")
    total_wig_sales = ds("total_wig_sales")
    total_repairs   = ds("total_repairs")
    total_other     = ds("total_other")
    total_revenue   = total_wash_set + total_wig_sales + total_repairs + total_other

    # ── Expenses (ExpenseEntry rows dated within the month) ──
    total_expenses = Decimal(str(
        db.query(func.coalesce(func.sum(ExpenseEntry.amount), 0))
          .filter(ExpenseEntry.expense_date >= start, ExpenseEntry.expense_date <= end)
          .scalar() or Decimal("0")
    ))

    # ── Payroll (WeeklyPayroll weeks whose start falls within the month) ──
    total_payroll = Decimal(str(
        db.query(func.coalesce(func.sum(WeeklyPayroll.amount), 0))
          .filter(WeeklyPayroll.week_start >= start, WeeklyPayroll.week_start <= end)
          .scalar() or Decimal("0")
    ))

    # ── Business logic: net profit, bank/owner split, tithes ──
