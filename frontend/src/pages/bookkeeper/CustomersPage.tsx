...
function CustomerModal({
  ...
  const [mapsReady, setMapsReady] = useState(() => !!(window as any).google?.maps?.places?.PlaceAutocompleteElement)
  const addressContainerRef = useRef<HTMLDivElement>(null)
  const addressValueRef = useRef(form.address)
  useEffect(() => { addressValueRef.current = form.address }, [form.address])

  // Load Maps JS API script once; set mapsReady when PlaceAutocompleteElement is available
  useEffect(() => {
    ...
    function onLoad() {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if ((window as any).google?.maps?.places?.PlaceAutocompleteElement) setMapsReady(true)
    }

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if ((window as any).google) { onLoad(); return }
  ...
  // Mount PlaceAutocompleteElement once Google is ready
  useEffect(() => {
    if (!mapsReady) return
    ...
    if (inner) {
      if (addressValueRef.current) inner.value = addressValueRef.current
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <Field label="Address">
            {mapsReady
              ...
              : <input value={form.address} onChange={e => set('address', e.target.value)} style={s.input} placeholder="Start typing to search address…" autoComplete="off" />
            }
          </Field>
...
type CustomerHistory = { pos_sales: PosSale[]; wig_sales: WigOrder[]; wig_pos_payments_total: number; pos_balance_payments_total: number }
...
function PurchaseHistoryModal({ customer, onClose }: { customer: Customer; onClose: () => void }) {
  ...
  const { data, isLoading } = useQuery<CustomerHistory>({
    ...
    queryFn: () => api.get(`/customers/${customer.id}/history`).then(r => r.data).catch(() => ({ pos_sales: [], wig_sales: [], wig_pos_payments_total: 0, pos_balance_payments_total: 0 })),
  })

  // Subtract balance payment totals to avoid double-counting:
  // - wig_pos_payments_total: wig balance payments already counted in both wig_sales and pos_sales
  // - pos_balance_payments_total: POS sale balance payments counted in both the original and checkout sale
  const totalSpent = (data?.pos_sales.reduce((s, p) => s + Number(p.amount_paid), 0) ?? 0)
    ...
    - (data?.pos_balance_payments_total ?? 0)
